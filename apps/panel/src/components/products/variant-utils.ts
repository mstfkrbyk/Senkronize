...
export function variantLabel(variant: ProductVariant): string {
  ...
  const parts = [attrs.Renk, attrs.Beden, ...Object.entries(attrs)
    .filter(([k]) => !['Renk', 'Beden', 'renk', 'beden'].includes(k))
    .map(([, v]) => v)]
    .filter(Boolean);
  return parts.length > 0 ? parts.join(' / ') : variant.title || variant.sku;
}
