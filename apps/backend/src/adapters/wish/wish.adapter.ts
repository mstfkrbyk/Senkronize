...
@Injectable()
export class WishAdapter extends RestStubMarketplaceAdapter {
  constructor(encryptionService: EncryptionService) {
    const opts: RestStubMarketplaceOptions = {
      platform: 'WISH',
      baseUrl: 'https://merchant.wish.com/api/v3',
      loggerContext: WishAdapter.name,
      rateLimitKey: 'WISH',
      ...
      resolveAuth: async (creds) => {
        ...
        if (!clientId || !clientSecret) {
          throw new Error(
            'Wish: clientId ve clientSecret (veya accessToken) zorunludur',
          );
        }
        const tokenUrl =
          creds.oauthTokenUrl?.trim() ??
          'https://merchant.wish.com/api/v3/oauth/token';
