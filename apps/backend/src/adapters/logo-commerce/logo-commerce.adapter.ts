...
@Injectable()
export class LogoCommerceAdapter implements IErpAdapter {
  readonly erpType = 'LOGO_COMMERCE';
  private readonly logger = new Logger(LogoCommerceAdapter.name);
  ...
  private async getToken(credentials: Record<string, string>): Promise<string> {
    const baseUrl = normalizeBaseUrl(
      credentials.baseUrl ?? 'https://api.logo.com.tr',
    );
    const username = credentials.username;
    const password = credentials.password;
    const firmNo = credentials.firmNo;
    const periodNo = credentials.periodNo;
    if (!username || !password || !firmNo || !periodNo) {
      throw new Error(
        'Logo Commerce: username, password, firmNo ve periodNo zorunludur',
      );
    }
    ...
    const { data } = await axios.post<LogoTokenResponse>(
      `${baseUrl}${LOGO_COMMERCE_API_PATH}/token`,
      {
        userName: username,
        password,
        firmNo,
  ...
  async getProducts(credentials: Record<string, string>): Promise<ErpProduct[]> {
    try {
      ...
      const { data } = await client.get<LogoItemsResponse>(
        `${LOGO_COMMERCE_API_PATH}/items`,
      ...
      this.logger.warn('Logo Commerce ürün listesi alınamadı', {
        error: error instanceof Error ? error.message : 'Bilinmeyen hata',
      });
      return [];
    }
  ...
  async createInvoice(
    ...
    const { data } = await client.post<LogoInvoiceCreateResponse>(
      `${LOGO_COMMERCE_API_PATH}/invoices`,
      body,
    );
