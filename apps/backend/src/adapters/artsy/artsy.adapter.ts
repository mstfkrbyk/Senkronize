...
@Injectable()
export class ArtsyAdapter extends RestStubMarketplaceAdapter {
  constructor(encryptionService: EncryptionService) {
    const opts: RestStubMarketplaceOptions = {
      platform: 'ARSY',
      baseUrl: 'https://api.artsy.net/api',
      loggerContext: ArtsyAdapter.name,
      rateLimitKey: 'ARSY',
      ...
      resolveAuth: async (creds) => {
        ...
        if (!clientId || !clientSecret) {
          throw new Error(
            'Artsy: clientId ve clientSecret (veya accessToken) zorunludur',
          );
        }
        const tokenUrl =
          creds.oauthTokenUrl?.trim() ??
          'https://api.artsy.net/api/oauth2/token';
