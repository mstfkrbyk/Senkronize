...
import { CICEKSEPETI_BASE_URL } from '../ciceksepeti/ciceksepeti.constants';
import type {
  CiceksepetiOrder,
...
export class CiceksepetiEvAdapter implements IMarketplaceAdapter {
  ...
  private categoryParams(
    ...
    const channel = credentials.categoryId?.trim() ?? credentials.channelId?.trim();
    if (!channel) {
      return {};
    }
  ...
  async testConnection(credentials: Record<string, string>): Promise<boolean> {
    try {
      ...
      this.logger.warn('Çiçeksepeti Ev bağlantı testi başarısız', {
  ...
  async getOrders(
    ...
    const params: Record<string, string | number> = {
      ...
      pageNumber: 0,
      pageSize: 100,
      ...this.categoryParams(credentials),
    };
