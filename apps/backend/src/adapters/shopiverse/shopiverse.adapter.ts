...
import type { ShopiverseJson } from './shopiverse.types';

const SHOPIVERSE_BASE = 'https://api.shopiverse.com/v1';

@Injectable()
export class ShopiverseAdapter implements IMarketplaceAdapter {
  readonly platform = 'SHOPIVERSE';
  private readonly logger = new Logger(ShopiverseAdapter.name);

  private rpm(): number {
    return PLATFORM_RATE_LIMITS.SHOPIVERSE ?? PLATFORM_RATE_LIMITS.DEFAULT;
  }
  ...
  async testConnection(credentials: Record<string, string>): Promise<boolean> {
    try {
      ...
      try {
        ...
        this.logger.warn('Shopiverse bağlantı testi başarısız', {
  ...
  async getOrders(
    ...
    try {
      ...
      return rows
        .map((row) => this.mapOrder(row as ShopiverseJson))
        .filter((o): o is MarketplaceOrder => o !== null);
    } catch (error) {
      this.logger.warn('Shopiverse sipariş çekme başarısız', {
        error: error instanceof Error ? error.message : 'Bilinmeyen hata',
      });
      return [];
    }
  ...
  private mapOrder(row: ShopiverseJson): MarketplaceOrder | null {
  ...
  async getListings(
    ...
    try {
      ...
      const items = rows
        .map((r) => this.mapListing(r as ShopiverseJson))
        .filter((l): l is MarketplaceListing => l !== null);
      return { items, total: total ?? items.length, page, pageSize };
    } catch (error) {
      this.logger.warn('Shopiverse ürün listesi başarısız', {
        error: error instanceof Error ? error.message : 'Bilinmeyen hata',
      });
  ...
  private mapListing(row: ShopiverseJson): MarketplaceListing | null {
    if (typeof row !== 'object' || row === null) {
      return null;
    }
    const p = row;
  ...
  async updateStock(
    ...
    for (const u of updates) {
      try {
        ...
        this.logger.warn('Shopiverse stok güncellemesi başarısız', {
          error: error instanceof Error ? error.message : 'Bilinmeyen hata',
        });
      }
    }
      ...
      try {
        await withRateLimit(this.platform, this.rpm(), async () => {
          await axiosWithRetry<unknown>(
            {
              method: 'PATCH',
              url: `${SHOPIVERSE_BASE}/products/price`,
              data: {
                sku: u.barcode,
        ...
        this.logger.warn('Shopiverse fiyat güncellemesi başarısız', {
          error: error instanceof Error ? error.message : 'Bilinmeyen hata',
        });
      }
    }
